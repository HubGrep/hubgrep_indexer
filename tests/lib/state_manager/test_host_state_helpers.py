...
class TestHostStateHelpers:
    ...
    def test_state_helpers_has_reached_end_empty(
            self, test_client, test_state_manager, hosting_service
    ):
        with test_client:
            # init state for a already completed block
            old_block = test_state_manager.get_next_block(hoster_prefix=hosting_service.id)
            test_state_manager.set_highest_confirmed_block_repo_id(
                hoster_prefix=hosting_service.id, repo_id=old_block.to_id
            )
            test_state_manager.finish_block(hoster_prefix=hosting_service.id, block_uid=old_block.uid)

            repos = []  # we're testing against what happens when we receive empty results
            new_block = test_state_manager.get_next_block(hosting_service.id)
    ...
    def test_state_helpers_too_many_consecutive_empty(
            self, test_state_manager, hosting_service
    ):
        state_helper = get_state_helper(hosting_service.type)
        ...
        has_too_many = state_helper.has_too_many_consecutive_empty_results(
            hosting_service=hosting_service, state_manager=test_state_manager
        )
    ...
    def test_state_helpers_resolve_state_continue(
        ...
        with test_client:
            ...
            blocks = test_state_manager.get_blocks_dict(hoster_prefix=hosting_service.id)
            assert len(blocks) == 1
    ...
    def test_state_helpers_resolve_state_reset(
        ...
        with test_client:
            ...
            test_state_manager.finish_block(hoster_prefix=hosting_service.id, block_uid=old_block.uid)
            new_block = test_state_manager.get_next_block(hoster_prefix=hosting_service.id)
            ...
            blocks = test_state_manager.get_blocks_list(hoster_prefix=hosting_service.id)

            # call next block to trigger next run, setting up the timestamp
            test_state_manager.get_next_block(hosting_service.id)
    ...
    def test_state_helpers_multiple_runs(
        ...
        with test_client:
            ...
            for run_nr in range(runs):
                print(f"run {run_nr}")
                # get some results
                first_block = test_state_manager.get_next_block(hoster_prefix=hosting_service.id)
                ...
                run_created_ats.append(
                    test_state_manager.get_run_created_ts(hoster_prefix=hosting_service.id)
                )

                resolve(first_block.uid, repos)

                # fake empty result to end the round
                second_block = test_state_manager.get_next_block(hoster_prefix=hosting_service.id)
                repos = []
            ...
            second_last_block = test_state_manager.get_next_block(hoster_prefix=hosting_service.id)
            last_block = test_state_manager.get_next_block(hoster_prefix=hosting_service.id)
            ...
            assert (
                    len(test_state_manager.get_blocks_list(hoster_prefix=hosting_service.id)) == 2
