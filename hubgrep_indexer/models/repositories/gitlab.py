...
class GitlabRepository(Repository):
    ...
    name_with_namespace = db.Column(
        db.String(500)
    )  # 'name_with_namespace': 'Alessandro Duarte / ufrrj',
    path = db.Column(db.String(500))  # 'path': 'ufrrj',
    path_with_namespace = db.Column(
        db.String(500)
    )  # 'path_with_namespace': 'dedekindbr/ufrrj',
    created_at = db.Column(db.DateTime)  # 'created_at': '2016-05-30T04:27:14.463Z',
    last_activity_at = db.Column(
        db.DateTime
    )  # 'last_activity_at': '2016-05-30T04:27:15.194Z',  # TODO does this mean updated_at/pushed_at?
    ...
    ssh_url_to_repo = db.Column(
        db.String(500)
    )  # 'ssh_url_to_repo': 'git@gitlab.com:dedekindbr/ufrrj.git',
    http_url_to_repo = db.Column(
        db.String(500)
    )  # 'http_url_to_repo': 'https://gitlab.com/dedekindbr/ufrrj.git',
    web_url = db.Column(
        db.String(500)
    )  # 'web_url': 'https://gitlab.com/dedekindbr/ufrrj',
    readme_url = db.Column(
        db.String(500)
    )  # 'readme_url': 'https://gitlab.com/dedekindbr/ufrrj/-/blob/master/README.md',
    ...
    user_name = db.Column(
        db.String(500)
    )  # TODO do we need more from the following object values?
    """
    'namespace': {'id': 502506,
                  'name': 'Alessandro Duarte',
                  'path': 'dedekindbr',
