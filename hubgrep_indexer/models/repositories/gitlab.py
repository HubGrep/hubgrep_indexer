...
class GitlabRepository(db.Model):
    ...
    name = db.Column(db.String(200))  # 'name': 'ufrrj',
    name_with_namespace = db.Column(db.String(200))  # 'name_with_namespace': 'Alessandro Duarte / ufrrj',
    path = db.Column(db.String(200))  # 'path': 'ufrrj',
    path_with_namespace = db.Column(db.String(200))  #'path_with_namespace': 'dedekindbr/ufrrj',
    ...
    default_branch = db.Column(db.String(200))  # 'default_branch': 'master',
    # tag_list = 'tag_list': [],  # TODO skipping for now, as its not important, and may need more tables
    ssh_url_to_repo = db.Column(db.String(200))  # 'ssh_url_to_repo': 'git@gitlab.com:dedekindbr/ufrrj.git',
    http_url_to_repo = db.Column(db.String(200))  # 'http_url_to_repo': 'https://gitlab.com/dedekindbr/ufrrj.git',
    web_url = db.Column(db.String(200))  # 'web_url': 'https://gitlab.com/dedekindbr/ufrrj',
    readme_url = db.Column(db.String(200))  # 'readme_url': 'https://gitlab.com/dedekindbr/ufrrj/-/blob/master/README.md',
    avatar_url = db.Column(db.String(200))  # 'avatar_url': None,
    forks_count = db.Column(db.String(200))  # 'forks_count': 0,
    star_count = db.Column(db.String(200))  # 'star_count': 0,
    user_name = db.Column(db.String(200))  # TODO do we need more from the following object values?
    """ 
    'namespace': {'id': 502506,
                  'name': 'Alessandro Duarte',
                  'path': 'dedekindbr',
